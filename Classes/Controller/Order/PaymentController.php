...
namespace Extcode\CartNexi\Controller\Order;
...
class PaymentController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    public function successAction()
    {
        if ($this->request->hasArgument('hash') && !empty($this->request->getArgument('hash'))) {
            ...
            if ($this->cart) {
                ...
                $this->addFlashMessage(
                    LocalizationUtility::translate(
                        'tx_cartnexi.controller.order.payment.action.success.error_occured',
                        $this->extensionName
                    ),
                    '',
            ...
            $this->addFlashMessage(
                LocalizationUtility::translate(
                    'tx_cartnexi.controller.order.payment.action.success.access_denied',
                    $this->extensionName
                ),
                '',
                \TYPO3\CMS\Core\Messaging\AbstractMessage::ERROR
    ...
    public function cancelAction()
    {
        if ($this->request->hasArgument('hash') && !empty($this->request->getArgument('hash'))) {
            ...
            if ($this->cart) {
                ...
                $this->addFlashMessage(
                    LocalizationUtility::translate(
                        'tx_cartnexi.controller.order.payment.action.cancel.successfully_canceled',
                        $this->extensionName
                    ),
                    '',
                ...
                $this->addFlashMessage(
                    LocalizationUtility::translate(
                        'tx_cartnexi.controller.order.payment.action.cancel.error_occured',
                        $this->extensionName
                    ),
                    '',
                    \TYPO3\CMS\Core\Messaging\AbstractMessage::ERROR
            ...
            $this->addFlashMessage(
                LocalizationUtility::translate(
                    'tx_cartnexi.controller.order.payment.action.cancel.access_denied',
                    $this->extensionName
                ),
                '',
                \TYPO3\CMS\Core\Messaging\AbstractMessage::ERROR
